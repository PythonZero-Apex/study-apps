...
SUBS = {
 "AIワークロードと考慮事項":["一般的なAIワークロードの識別","責任あるAIの原則"],
 "機械学習の基本原則":["一般的な機械学習の手法","機械学習のコア概念","Azure Machine Learningの機能"],
 "コンピュータービジョン":["CVソリューションの種別","CVのAzureサービス"],
 "自然言語処理（NLP）":["NLPワークロードのシナリオ","NLPのAzureサービス"],
 "生成AI":["生成AIソリューションの特徴","生成AIのAzureサービスと機能"],
}
...
for i, q in enumerate(raw, start=1):
    ...
    ans = []
    for c in qq["correct"]:
        if c not in opts: errors.append(f"id{i}: correctが選択肢にない '{c}' / {opts}")
        else: ans.append(opts.index(c))
    ans.sort(); qq["answer"] = ans
    if qq["type"]=="single" and len(ans)!=1: errors.append(f"id{i}: singleなのに正解{len(ans)}個")
    if qq["type"]=="multi" and len(ans)<2: errors.append(f"id{i}: multiなのに正解{len(ans)}個")
...
for a in range(len(texts)):
    for b in range(a+1,len(texts)):
        ...
        if r>0.90: errors.append(f"類似度{r:.2f}: id{texts[a][0]} ~ id{texts[b][0]}")
...
if errors:
    ...
    sys.exit(1)
print("OK: エラーなし")
with open("ai_template.html",encoding="utf-8") as f: tpl=f.read()
